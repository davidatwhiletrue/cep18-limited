...
use casper_types::{
    bytesrepr::ToBytes, contracts::NamedKeys, runtime_args, CLValue,
    Key, U256,
};
...
use constants::{
    ...
    CHANGE_EVENTS_MODE_ENTRY_POINT_NAME, CONTRACT_NAME_PREFIX,
...
use crate::utils::get_optional_named_arg_with_user_errors;
...
pub extern "C" fn approve() {
    let owner = utils::get_immediate_caller_address()
...
pub extern "C" fn transfer() {
    let sender = utils::get_immediate_caller_address()
        .unwrap_or_revert_with(Cep18Error::FailedToRetrieveImmediateCaller);
...
pub extern "C" fn transfer_from() {
    let spender = utils::get_immediate_caller_address()
        .unwrap_or_revert_with(Cep18Error::FailedToRetrieveImmediateCaller);
    let recipient: Key = runtime::get_named_arg(RECIPIENT);
    let owner: Key = runtime::get_named_arg(OWNER);
    if owner == recipient {
...
pub extern "C" fn burn() {
    ...
    if owner != get_immediate_caller_address().unwrap_or_revert_with(Cep18Error::FailedToRetrieveImmediateCaller)
    {
...
pub fn upgrade(name: &str) {
    ...
    let contract_package_hash: ContractPackageHash = runtime::get_key(&format!("{HASH_KEY_NAME_PREFIX}{name}"))
        .unwrap_or_revert_with(Cep18Error::MissingPackageHashForUpgrade).into_hash_addr().unwrap().into();

    let previous_contract_hash: ContractHash = runtime::get_key(&format!("{CONTRACT_NAME_PREFIX}{name}"))
        .unwrap_or_revert_with(Cep18Error::MissingPackageHashForUpgrade).into_hash_addr().unwrap().into();
    ...
    // TODO: do this only if the topic hasn't been inserted yet in previous version...
...
pub fn install_contract(name: &str) {
    ...
    let mut init_args = runtime_args! {TOTAL_SUPPLY => total_supply, PACKAGE_HASH => package_hash, EVENTS_MODE => events_mode};
